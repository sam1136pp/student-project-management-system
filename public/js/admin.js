...
// Section navigation — fixed: accepts event as explicit parameter
...
async function loadUsers() {
    try {
        ...
        tbody.innerHTML = users.map(u => {
            let actions = u.user_id !== currentUser.user_id
                ? '<button class="btn btn-danger btn-sm" onclick="deleteUser(' + u.user_id + ', \'' + escHtml(u.name).replace(/'/g, "\\'") + '\')">Delete</button>'
                : '<span style="color:var(--text-muted);font-size:12px">You</span>';
            return '<tr><td><strong>' + escHtml(u.name) + '</strong></td><td>' + escHtml(u.username) + '</td><td>' + escHtml(u.email) + '</td><td><span class="badge badge-' + u.role + '">' + u.role + '</span></td><td>' + escHtml(u.department || '\u2014') + '</td><td>' + actions + '</td></tr>';
        }).join('');
    } catch (e) {
...
async function loadProjects() {
    try {
        ...
        tbody.innerHTML = projects.map(p => {
            let mentor = p.faculty_name ? escHtml(p.faculty_name) : '<span style="color:var(--accent-yellow)">Not assigned</span>';
            return '<tr><td><strong>' + escHtml(p.title) + '</strong></td><td>' + escHtml(p.student_name) + '</td><td>' + mentor + '</td><td>' + escHtml(p.technology || '\u2014') + '</td><td><span class="badge badge-' + p.status + '">' + p.status + '</span></td><td><button class="btn btn-outline btn-sm" onclick="openAssignModal(' + p.project_id + ')">Assign Mentor</button></td></tr>';
        }).join('');
    } catch (e) {
...
async function loadFacultyList() {
    try {
        const res = await fetch(`${API_BASE}/api/admin/faculty`, { credentials: 'include' });
...
document.getElementById('assignForm').addEventListener('submit', async (e) => {
    ...
    try {
        const res = await fetch(`${API_BASE}/api/admin/projects/${projectId}/assign`, {
            ...
            credentials: 'include', body: JSON.stringify({ faculty_id: parseInt(facultyId) })
        });
        const data = await res.json();
        if (res.ok) {
